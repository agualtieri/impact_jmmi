...
df2 <- new_df <- rbind(month1, month2) 
...
```{r tren analysis, message = FALSE, warning = FALSE}

df2$month_date<- lubridate::month(new_df$month)

pct_change <- df2 %>% gropu_bypct_change_by_groups_all_numerics(df = new_df,group_var = district, time_id = month_date)
