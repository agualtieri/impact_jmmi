...
For further inquires reach out to Chris (chris.paci@reach-initiative.org) and Alberto (alberto.gualtieri@impact-initiatives.org).
...
The minimum standards function allows you to quickly check how many markets per area of interest have been collected by the enumerators. The function returns a table with the number of market collected aggregated by your variable of interest (it can be subdsitrict, district, or governorate).
...
A "Silhouette Analysis" is based on the "gower" distance between surveys which is a metric that measures the dissimilarities between two items with mixed numeric and non-numeric data. This method checks for anomalies assuming the datast is clustered around the enumerator ID. A silhouette value of 1 indicats that the entries of the cluster are very similar to each other, while a value close to 0 or negative indicates the opposite. Values close to 1 should raise a flag as it means that enumerators are filling surveyes very similar to one another.
...
The first step to calculate the SMEB value is to aggregate prices using the median-of-medians methodology. This methodology ensures a balanced aggregation reducing the variance created by having markets largerly more surveyed compared to others.
...
print(head(median_items, 3))                                              # the output is an aggregation by governorate, district, and markets

median_items_district <- data %>%                                                                            
                        select(-uuid, -enum_id, -index, -market) %>% group_by(governorate, district) %>%     
                        summarise_all(funs(median(., na.rm = TRUE)))  

print(head(median_items_district, 3))                                     # the output is an aggregation by governorate, and district
```

### SMEB calculation

Below we show a few lines of code you can use to quickly calculate the SMEB value after having calculated the median price per item. The calculation will be different based on the weights and the items included in the SMEB. If you nee to split the calcul
...
To correctly produce a trend analysis the two datasets that are compared need to balanced, meaning that the unit compare must be the same. 
...
data2$month_date<- lubridate::month(data2$month)

# Run the percentag change formula taken from the butteR package
pct_change <- data2 %>% pct_change_by_groups_all_numerics(group_var = district, time_id = month_date)
